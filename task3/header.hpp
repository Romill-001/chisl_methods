...
vector<double> calculate_mtx_vec_mult(vector<vector<double>>& A,  vector<double> &x) {
    int n = A.size();
    int m = x.size();
...
double calculate_error_dec(vector<double> &x, vector<vector<double>> &A, vector<double> &b) {
...
int Seidel_method(int n, vector<double> &x, vector<vector<double>> &A, vector<double> &b) {
    double h = 1.0 / n;
    int k = 0;
